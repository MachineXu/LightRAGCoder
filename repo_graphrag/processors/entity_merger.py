...
async def _batch_embedding(embedding_func, texts: List[str], batch_size: int = embedding_batch_size) -> List[List[float]]:
    """
    ...
    logger.info(f"Processing embeddings in batches: {len(texts)} texts, batch size: {batch_size}, total batches: {total_batches}")
    ...
    logger.info(f"Embedding complete: processed {len(all_embeddings)} embeddings")
...
async def merge_doc_and_code(rag: LightRAG, current_code_dict: Optional[Dict[str, Any]] = None) -> None:
    ...
    # Embed code and document entity names in batches
    embedding_func = rag.embedding_func

    # Process code entity names in batches
    code_name_embedding_array = await _batch_embedding(embedding_func, code_definition_name_list, embedding_batch_size)

    # Process document entity names in batches
    doc_names = [doc_name for doc_name, _ in all_doc_entity_list]
    doc_name_embedding_array = await _batch_embedding(embedding_func, doc_names, embedding_batch_size)
